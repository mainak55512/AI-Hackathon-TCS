    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <nav className="app-nav">
        <div className="nav-container">
          ...
          {isDashboard && (
            <button onClick={() => navigate('/fraudlog')} className="btn-logout">
              Back
            </button>)}
    ...
    <BrowserRouter>
      ...
      <div className="app-root">
        <Routes>
          ...
          <Route 
            path="/login" 
            element={!user ? <LoginPage onLoginSuccess={(u) => setUser(u)} /> : <Navigate to="/fraudlog" replace />} 
          />
          ...
          {/* DEFAULT REDIRECT */}
          <Route path="/" element={<Navigate to={user ? "/fraudlog" : "/login"} replace />} />
          <Route path="*" element={<Navigate to={user ? "/fraudlog" : "/login"} replace />} />
        </Routes>
      </div>
    </BrowserRouter>
