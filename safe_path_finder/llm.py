...
def get_safest_path(start, end):
    [safest_path, reason] = get_path(start, end)
    output = " ------> ".join(map(surround, safest_path))
    output += "\n\nReason: " + reason
    return output
...
def get_available_paths(start, end):
    path_id = " - ".join([start, end])
    # print("Path ID: ", path_id)
    return available_paths.available_paths[path_id]
...
def get_path(start, end):
    ...
    all_path = [
        path_details_builder(df, path)
        for path in get_available_paths(start, end)["paths"]
    ]
    ...
    **Decision Logic:**
    ...
    2. Calculate safety using: Higher 'lux', 'footfall' and 'cctv_coverage' are positive; higher 'incident_density' is strongly negative.
    ...
    **CRITICAL INSTRUCTIONS:**
    ...
    return content["safest_path"], content["reason"]


if __name__ == "__main__":
    # app.run(debug=True)
    # get_available_paths("KOL_002", "KGP_010")
    print(get_safest_path())
